...
module.exports = function (app) {
  ...
  app
    ...
    .post(function (req, res){
      ...
      ProjectModel.findOne({name: project}, (err, prdata) => {
          if(!prdata || err){
              const newProject = new ProjectModel({name: project});
              newProject.issues.push(newIssue);
              newProject.save((err, data) => {
                  if(err || !data){
                      res.send ("There was a error saving the post");
                  } else {
                      res.json(newIssue)
                  }
              });
          } else {
              prdata.issues.push(newIssue);
              prdata.save((err, data) => {
                  if(err || !data){
                      res.send("There was a error saving the post");
                  } else {
                      res.json(newIssue);
    ...
    .put(function (req, res){
      ...
      if(
          !issue_text && !issue_text && !created_by && !assigned_to && !status_text && !open
      ) {
